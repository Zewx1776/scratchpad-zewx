...
local function logics(target)
    ...
    local is_logic_allowed = my_utility.is_spell_allowed(
        ...
        spell_data.the_hunter.spell_id);

    if not is_logic_allowed then return false end;

    -- if we have the buff already active then skip
    if my_utility.is_buff_active(spell_data.the_hunter.spell_id, spell_data.the_hunter.buff_id) then
        return false;
    end;
    ...
    if cast_spell.position(spell_data.the_hunter.spell_id, target_position, 0.40) then
        local current_time = get_time_since_inject();
        next_time_allowed_cast = current_time + my_utility.spell_delays.regular_cast;
        console.print("Cast The Hunter, Target: " .. my_utility.targeting_modes
            [menu_elements.targeting_mode:get() + 1] .. ", Mobility Only: " .. tostring(mobility_only));
        return true;
    end
