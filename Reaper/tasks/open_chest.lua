...
-- ---- Config ----
local CHEST_INTERACT_COOLDOWN = 1    -- min seconds between interact attempts
local WAIT_GONE_SECS          = 3    -- if chest still here after this → out of mats
local THEME_WAIT_SECS         = 8    -- seconds to look for theme chest
local WAIT_COMPLETE_SECS      = 1    -- pause after all chests before next run
    ...
    if phase == "THEME" then
        ...
        if not chest then
            ...
            if last_chest_pos and utils.distance_to(last_chest_pos) > 2.5 then
                BatmobilePlugin.set_target(plugin_label, last_chest_pos)
                BatmobilePlugin.update(plugin_label)
                BatmobilePlugin.move(plugin_label)
            end
            return
