...
local function is_action_allowed()
   ...
   local active_spell_id = local_player:get_active_spell_id()
    for index, value in ipairs(blacklist) do
        if value == active_spell_id then
            return false
        end
    end
  
    local current_gametime = get_gametime();
    ...
    for _, buff in ipairs(local_player_buffs) do
        ...
        -- console.print("buff name ", buff:name(), "buff end time ", buff:get_end_time(), "gametime ", get_gametime());
        if buff.name_hash == blood_mist_buff_name_hash_c then
            local end_time = buff:get_end_time() - 3.0;
            local is_active = current_gametime < end_time;
            if is_active then
                is_blood_mist = true;
                break;
            end            
...
local plugin_label = "BASE_NECROMANCER_PLUGIN_"
