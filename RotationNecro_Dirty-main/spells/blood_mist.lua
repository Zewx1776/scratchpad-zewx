...
local menu_elements =
{
    ...
    main_boolean          = checkbox:new(true, get_hash(my_utility.plugin_label .. "main_boolean_blood_mist")),
    hp_usage_shield       = slider_float:new(0.0, 1.0, 0.60,
        get_hash(my_utility.plugin_label .. "%_blood_mist_hp_usage")),
    use_offensively       = checkbox:new(false, get_hash(my_utility.plugin_label .. "use_offensively_blood_mist")),
    filter_mode           = combo_box:new(1, get_hash(my_utility.plugin_label .. "offensive_filter_blood_mist")),
    enemy_count_threshold = slider_int:new(0, 30, 5, get_hash(my_utility.plugin_label .. "min_enemy_count_blood_mist")),
    check_buff            = checkbox:new(true, get_hash(my_utility.plugin_label .. "check_buff_blood_mist")),
    evaluation_range      = slider_int:new(1, 16, 6,
        get_hash(my_utility.plugin_label .. "evaluation_range_blood_mist")),
}

local function menu()
    if menu_elements.tree_tab:push("Blood Mist") then
...
local function logics()
    ...
    local is_logic_allowed = my_utility.is_spell_allowed(
        ...
        spell_data.blood_mist.spell_id);
    ...
    if check_buff then
        local is_buff_active = my_utility.is_buff_active(spell_data.blood_mist.spell_id,
            spell_data.blood_mist.buff_ids.base);
    ...
    if menu_min_percentage < 1 then
        ...
        if health_percent <= menu_min_percentage then
            if cast_spell.self(spell_data.blood_mist.spell_id, 0) then
                local current_time = get_time_since_inject();
                next_time_allowed_cast = current_time + my_utility.spell_delays.regular_cast;
                console.print("Cast Blood Mist - Defensive - " .. string.format("%.1f", health_percent))
                return true;
            end
        end
    else
        if cast_spell.self(spell_data.blood_mist.spell_id, 0) then
            local current_time = get_time_since_inject();
            next_time_allowed_cast = current_time + my_utility.spell_delays.regular_cast;
            console.print("Cast Blood Mist - Defensive")
            return true;
        end
    end
    ...
    if use_offensively then
        ...
        then
            if cast_spell.self(spell_data.blood_mist.spell_id, 0) then
                local current_time = get_time_since_inject();
                next_time_allowed_cast = current_time + my_utility.spell_delays.regular_cast;
                console.print("Cast Blood Mist - Offensive - " .. my_utility.activation_filters[filter_mode + 1])
                return true;
            end
        end
    end
