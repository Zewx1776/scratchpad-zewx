...
local menu_elements =
{
    ...
    use_offensively       = checkbox:new(true, get_hash(my_utility.plugin_label .. "use_offensively_cyclone_armor")),
    ...
    check_buff            = checkbox:new(true, get_hash(my_utility.plugin_label .. "check_buff_cyclone_armor")),
...
local function logics()
    ...
    if check_buff then
        local is_buff_active = my_utility.is_buff_active(spell_data.cyclone_armor.spell_id,
            spell_data.cyclone_armor.buff_id);
    ...
    if menu_min_percentage < 1 then
        ...
        if cast_spell.self(spell_data.cyclone_armor.spell_id, 0) then
            local current_time = get_time_since_inject();
            next_time_allowed_cast = current_time + my_utility.spell_delays.regular_cast;
            console.print("Cast Cyclone Armor - Defensive")
            return true;
        end
    end
