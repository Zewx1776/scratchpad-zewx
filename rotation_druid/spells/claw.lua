...
local menu_elements_claw = 
{
    ...
    main_boolean          = checkbox:new(false, get_hash(my_utility.plugin_label .. "claw_main_bool_base")),
...
local function menu()
    ...
    if menu_elements_claw.tree_tab:push("Claw") then
        ...
        if menu_elements_claw.main_boolean:get() then
            menu_elements_claw.targeting_mode:render("Targeting Mode", my_utility.targeting_modes, my_utility.targeting_mode_description)
...
local function logics(target)
    ...
    if cast_spell.target(target, spell_data.claw.spell_id, 0, false) then
        ...
        console.print("Cast Claw - Target: " ..
