...
local menu_elements =
{
    ...
    main_boolean          = checkbox:new(false, get_hash(my_utility.plugin_label .. "main_boolean_debilitating_roar")),
    hp_usage_shield       = slider_float:new(0.0, 1.0, 0.80,
        get_hash(my_utility.plugin_label .. "%_debilitating_roar_hp_usage")),
    use_offensively       = checkbox:new(true, get_hash(my_utility.plugin_label .. "use_offensively_debilitating_roar")),
    ...
    check_buff            = checkbox:new(true, get_hash(my_utility.plugin_label .. "check_buff_debilitating_roar")),
...
local function logics()
    ...
    if check_buff then
        local is_buff_active = my_utility.is_buff_active(spell_data.debilitating_roar.spell_id,
            spell_data.debilitating_roar.buff_id);
    ...
    if menu_min_percentage < 1 then
        ...
        if cast_spell.self(spell_data.debilitating_roar.spell_id, 0) then
            local current_time = get_time_since_inject();
            next_time_allowed_cast = current_time + my_utility.spell_delays.regular_cast;
            console.print("Cast Debilitating Roar - Defensive")
            return true;
        end
    end
