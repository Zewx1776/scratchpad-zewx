...
local menu_elements =
{
    ...
    main_boolean   = checkbox:new(false, get_hash(my_utility.plugin_label .. "pulverize_base_main_bool")),
    targeting_mode = combo_box:new(3, get_hash(my_utility.plugin_label .. "pulverize_base_targeting_mode")),
}

local function menu()
    if menu_elements.tree_tab:push("Pulverize") then
...
local function logics(target)
    ...
    if cast_spell.target(target, spell_data.pulverize.spell_id, 0, false) then
        ...
        console.print("Cast Pulverize - Target: " ..
