...
local menu_elements =
{
    ...
    main_boolean          = checkbox:new(false, get_hash(my_utility.plugin_label .. "main_boolean_earthen_bulwark")),
    hp_usage_shield       = slider_float:new(0.0, 1.0, 0.80,
        get_hash(my_utility.plugin_label .. "%_earthen_bulwark_hp_usage")),
    use_offensively       = checkbox:new(true, get_hash(my_utility.plugin_label .. "use_offensively_earthen_bulwark")),
    filter_mode           = combo_box:new(1, get_hash(my_utility.plugin_label .. "offensive_filter_earthen_bulwark")),
    enemy_count_threshold = slider_int:new(0, 30, 5, get_hash(my_utility.plugin_label .. "min_enemy_count_earthen_bulwark")),
    check_buff            = checkbox:new(true, get_hash(my_utility.plugin_label .. "check_buff_earthen_bulwark")),
    evaluation_range      = slider_int:new(1, 16, 6,
        get_hash(my_utility.plugin_label .. "evaluation_range_earthen_bulwark")),
}

local function menu()
    if menu_elements.tree_tab:push("Earthen Bulwark") then
...
local function logics()
    ...
    if check_buff then
        local is_buff_active = my_utility.is_spell_active(spell_data.earthen_bulwark.spell_id);
    ...
    if menu_min_percentage < 1 then
        ...
        if health_percent <= menu_min_percentage then
            if cast_spell.self(spell_data.earthen_bulwark.spell_id, 0) then
                local current_time = get_time_since_inject();
                next_time_allowed_cast = current_time + my_utility.spell_delays.regular_cast;
                console.print("Cast Earthen Bulwark - Defensive - " .. string.format("%.1f", health_percent))
                return true;
            end
        end
    else
        if cast_spell.self(spell_data.earthen_bulwark.spell_id, 0) then
            local current_time = get_time_since_inject();
            next_time_allowed_cast = current_time + my_utility.spell_delays.regular_cast;
            console.print("Cast Earthen Bulwark - Defensive")
            return true;
        end
    end
    ...
    if use_offensively then
        ...
        then
            if cast_spell.self(spell_data.earthen_bulwark.spell_id, 0) then
                local current_time = get_time_since_inject();
                next_time_allowed_cast = current_time + my_utility.spell_delays.regular_cast;
                console.print("Cast Earthen Bulwark - Offensive - " .. my_utility.activation_filters[filter_mode + 1])
                return true;
            end
        end
    end
