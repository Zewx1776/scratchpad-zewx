...
local menu_elements = {
    ...
    -- Filtered Actor Print
    filtered_actor_print_tree = tree_node:new(1),
    enable_filtered_actor_print = checkbox:new(false, get_hash("debug_module_enable_filtered_actor_print_checkbox")),
    filtered_actor_print_filter = input_text:new(get_hash("debug_module_filtered_actor_print_filter_input")),
    filtered_actor_print_min_dist = slider_float:new(0.0, 100.0, 0.0, get_hash("debug_module_filtered_actor_print_min_dist_slider")),
    filtered_actor_print_max_dist = slider_float:new(0.0, 100.0, 20.0, get_hash("debug_module_filtered_actor_print_max_dist_slider")),
    filtered_actor_print_interval = slider_float:new(0.5, 10.0, 2.0, get_hash("debug_module_filtered_actor_print_interval_slider")),
}
...
local function render_menu()
    if menu_elements.main_tree:push("Debug Tools") then
        ...
        if menu_elements.filtered_actor_print_tree:push("Filtered Actor Print") then
            menu_elements.enable_filtered_actor_print:render("Enable", "Print actors matching filter within distance range to console")
            menu_elements.filtered_actor_print_filter:render("Name Filter", "Only print actors whose name contains this text", true, "Go to Input", "Actor Name Filter")
            menu_elements.filtered_actor_print_min_dist:render("Min Distance", "Minimum distance from player", 1)
            menu_elements.filtered_actor_print_max_dist:render("Max Distance", "Maximum distance from player", 1)
            menu_elements.filtered_actor_print_interval:render("Print Interval (s)", "Seconds between console prints", 1)
            menu_elements.filtered_actor_print_tree:pop()
        end
...
-- Filtered actor print cooldown
local last_filtered_print_time = 0

local function print_filtered_actors()
    if not menu_elements.enable_filtered_actor_print:get() then
        return
    end

    local now = os.clock()
    local interval = menu_elements.filtered_actor_print_interval:get()
    if now - last_filtered_print_time < interval then
        return
    end
    last_filtered_print_time = now

    local local_player = get_local_player()
    if not local_player then return end

    local player_pos = get_player_position()
    local min_dist = menu_elements.filtered_actor_print_min_dist:get()
    local max_dist = menu_elements.filtered_actor_print_max_dist:get()
    local filter_text = string.lower(menu_elements.filtered_actor_print_filter:get())
    ...
    for _, actor in ipairs(all_actors) do
        ...
        if filter_text == "" or string.find(lower_name, filter_text) then
            local actor_pos = actor:get_position()
            local dist_sqr = actor_pos:squared_dist_to_ignore_z(player_pos)

            if dist_sqr >= (min_dist * min_dist) and dist_sqr <= (max_dist * max_dist) then
                local dist = math.sqrt(dist_sqr)
                console.print(string.format("[FilteredActor] %s | ID: %s | Dist: %.1f", name, tostring(actor:get_type_id()), dist))
                count = count + 1
            end
        end
    end
    ...
    if count > 0 then
        console.print(string.format("[FilteredActor] -- %d actors matched --", count))
    end
end
