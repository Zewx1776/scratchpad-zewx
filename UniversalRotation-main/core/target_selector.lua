...
local function _is_in_sight(from_pos, to_pos)
    if not (prediction and type(prediction.is_wall_collision) == 'function') then return true end
    if not (from_pos and to_pos) then return true end
    local ok, blocked = pcall(prediction.is_wall_collision, from_pos, to_pos, 1)
...
function target_selector.pick_target(targets, spell_cfg, player_pos, range)
    ...
    local check_sight = spell_cfg and spell_cfg.require_sight

    local function in_range(enemy)
        if not r2 then return true end
        local epos = _pos(enemy)
        if not epos then return false end
        if _dist2(epos, player_pos) > r2 then return false end
        if check_sight and not _is_in_sight(player_pos, epos) then return false end
        return true
    end
