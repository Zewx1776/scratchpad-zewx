...
local function get_elements(spell_id)
    ...
    local e = {
        ...
        -- Line of sight: require target to not be behind a wall
        require_sight   = checkbox:new(false, get_hash(key(spell_id, 'require_sight'))),
...
function spell_config.render(spell_id, display_name, equipped_ids, all_known_ids)
    ...
    -- Line of Sight
    if not is_self then
        e.require_sight:render('Require Line of Sight', 'Only target enemies not blocked by walls (uses wall collision check)')
    end
...
function spell_config.get(spell_id)
    ...
    return {
        ...
        require_sight   = e.require_sight:get(),
...
function spell_config.apply(spell_id, cfg)
    ...
    _set_element(e.require_sight, cfg.require_sight)
