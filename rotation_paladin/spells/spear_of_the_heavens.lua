...
local menu_elements = {
    ...
    main_boolean = checkbox:new(false, get_hash(my_utility.plugin_label .. "spear_of_the_heavens_main_bool")),
    min_range = slider_float:new(0.0, 25.0, 20.0, get_hash(my_utility.plugin_label .. "spear_of_the_heavens_min_range")),
    min_enemies_aoe = slider_int:new(0, 10, 1, get_hash(my_utility.plugin_label .. "spear_of_the_heavens_min_enemies")),
    cooldown = slider_float:new(0.0, 2.0, 0.3, get_hash(my_utility.plugin_label .. "spear_of_the_heavens_cooldown")),
}

local function menu()
    if menu_elements.tree_tab:push("Spear Of The Heavens") then
        menu_elements.main_boolean:render("Enable Spell", "Ranged spear attack")
...
local spell_id = 2100457
local next_time_allowed_cast = 0.0

local spell_data_spearoftheheavens = spell_data:new(
    1.5,                        -- radius
    20.0,                       -- range
    0.3,                       -- cast_delay
...
local function logics(target)
    ...
    if cast_spell.target(target, spell_data_spearoftheheavens, false) then
