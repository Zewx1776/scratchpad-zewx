...
local menu_elements = {
    ...
    main_boolean = checkbox:new(false, get_hash(my_utility.plugin_label .. "consecration_main_bool")),
    min_range = slider_float:new(0.0, 25.0, 12.0, get_hash(my_utility.plugin_label .. "consecration_min_range")),
    min_enemies_aoe = slider_int:new(0, 10, 1, get_hash(my_utility.plugin_label .. "consecration_min_enemies")),
    cooldown = slider_float:new(0.0, 2.0, 0.5, get_hash(my_utility.plugin_label .. "consecration_cooldown")),
}

local function menu()
    if menu_elements.tree_tab:push("Consecration") then
        menu_elements.main_boolean:render("Enable Spell", "Ground effect damage")
...
local spell_id = 2283781
local next_time_allowed_cast = 0.0

local spell_data_consecration = spell_data:new(
    5.0,                        -- radius
    12.0,                       -- range
    0.5,                       -- cast_delay
...
local function logics(target)
    ...
    if cast_spell.target(target, spell_data_consecration, false) then
