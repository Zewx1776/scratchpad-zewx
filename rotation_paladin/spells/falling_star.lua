...
local menu_elements = {
    ...
    main_boolean = checkbox:new(false, get_hash(my_utility.plugin_label .. "falling_star_main_bool")),
    min_range = slider_float:new(0.0, 25.0, 15.0, get_hash(my_utility.plugin_label .. "falling_star_min_range")),
    min_enemies_aoe = slider_int:new(0, 10, 1, get_hash(my_utility.plugin_label .. "falling_star_min_enemies")),
    cooldown = slider_float:new(0.0, 2.0, 0.3, get_hash(my_utility.plugin_label .. "falling_star_cooldown")),
}

local function menu()
    if menu_elements.tree_tab:push("Falling Star") then
        menu_elements.main_boolean:render("Enable Spell", "Ranged star attack")
...
local spell_id = 2106904
local next_time_allowed_cast = 0.0

local spell_data_fallingstar = spell_data:new(
    3.0,                        -- radius
    20.0,                       -- range
    0.4,                       -- cast_delay
...
local function logics(target)
    ...
    if cast_spell.target(target, spell_data_fallingstar, false) then
