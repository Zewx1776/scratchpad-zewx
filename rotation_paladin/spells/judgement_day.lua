...
local menu_elements = {
    ...
    main_boolean = checkbox:new(false, get_hash(my_utility.plugin_label .. "judgement_day_main_bool")),
    min_range = slider_float:new(0.0, 25.0, 15.0, get_hash(my_utility.plugin_label .. "judgement_day_min_range")),
    min_enemies_aoe = slider_int:new(0, 10, 1, get_hash(my_utility.plugin_label .. "judgement_day_min_enemies")),
    cooldown = slider_float:new(0.0, 2.0, 0.5, get_hash(my_utility.plugin_label .. "judgement_day_cooldown")),
}

local function menu()
    if menu_elements.tree_tab:push("Judgement Day") then
        menu_elements.main_boolean:render("Enable Spell", "High damage skill")
...
local spell_id = 2425899
local next_time_allowed_cast = 0.0

local spell_data_judgementday = spell_data:new(
    2.0,                        -- radius
    15.0,                       -- range
    0.4,                       -- cast_delay
...
local function logics(target)
    ...
    if cast_spell.target(target, spell_data_judgementday, false) then
