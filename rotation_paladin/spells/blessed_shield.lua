...
local menu_elements = {
    ...
    main_boolean = checkbox:new(false, get_hash(my_utility.plugin_label .. "blessed_shield_main_bool")),
    min_range = slider_float:new(0.0, 25.0, 15.0, get_hash(my_utility.plugin_label .. "blessed_shield_min_range")),
    min_enemies_aoe = slider_int:new(0, 10, 1, get_hash(my_utility.plugin_label .. "blessed_shield_min_enemies")),
    cooldown = slider_float:new(0.0, 2.0, 0.3, get_hash(my_utility.plugin_label .. "blessed_shield_cooldown")),
}

local function menu()
    if menu_elements.tree_tab:push("Blessed Shield") then
        menu_elements.main_boolean:render("Enable Spell", "Shield throw attack")
...
local spell_id = 2082021
local next_time_allowed_cast = 0.0

local spell_data_blessedshield = spell_data:new(
    1.0,                        -- radius
    15.0,                       -- range
    0.25,                       -- cast_delay
...
local function logics(target)
    ...
    if cast_spell.target(target, spell_data_blessedshield, false) then
